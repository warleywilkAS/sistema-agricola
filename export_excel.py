...
def _calcular_contagem_pragas(registros: list[dict]):
    """
    Uma linha por registro (questionario), com 3 colunas por praga/doenca
    (mesmos 21 grupos usados na Medias_Geral): Aplicou?, Nº Aplicações,
    DAE 1ª Aplicação. Versao condensada do que seria a Contagem_Pragas
    original (534 colunas cruas, uma por praga individual x pulverizacao).
    """
    headers_id = ["N° Produtor", "Região", "Mesorregião", "Município", "Bt"]
    headers_alvo = []
    for nome, _lista in _BLOCOS_ALVO:
    ...
    for reg in registros:
        ...
        for _nome, lista in _BLOCOS_ALVO:
