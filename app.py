...
import os
from export_excel import gerar_excel, orm_para_dict, ALL_COLS, _calcular_medias_geral, REGIOES_COLUNAS, _calcular_contagem_pragas, _calcular_distribuicao_alvo, _LAGARTAS, _PERCEVEJOS, _OUTRAS_PRAGAS, ACAROS, _calcular_fbn, _calcular_tto_sal_cb, _calcular_ins_dess, _calcular_herbicidas, DOENCAS, _calcular_contagem_doencas, _calcular_grafico_mip, _calcular_grafico_mid, _OUTRAS_PRAGAS, ACAROS
...
def get_safras_existentes():
    """Lista as safras que já têm pelo menos um registro salvo, mais recente primeiro."""
    return [
...
def view_records():
    ...
    return render_template('view_records.html',
                            registros=registros,
                            safras=get_safras_existentes(),
                            safra_selecionada=safra_filtro)

@app.route('/painel')
def painel():
    safra_filtro = request.args.get('safra')
    safras_existentes = get_safras_existentes()
    if not safra_filtro and safras_existentes:
        safra_filtro = safras_existentes[0]  # mais recente, por padrão

    query = FormularioSoja.query
    if safra_filtro:
        query = query.filter_by(safra=safra_filtro)
    ...
    return render_template('painel.html',
                            ...
                            regioes_mg=[r for r in REGIOES_COLUNAS if r])

# Colunas reais da Total_Pr (mesma lista usada na exportação Excel),
# ignorando as colunas estruturais (sequência, "TB1.", "Ordem", branco final)
_TOTAL_PR_COLS_WEB = [(key, label) for key, label, _fmt in ALL_COLS if key and key != "_TABELA"]

@app.route('/painel/dados/total_pr')
def painel_dados_total_pr():
    safra = request.args.get('safra')
    query = FormularioSoja.query.order_by(FormularioSoja.id)
    if safra:
        query = query.filter_by(safra=safra)
    registros = [orm_para_dict(r) for r in query.all()]

    linhas = []
    for reg in registros:
        linhas.append([reg.get(key) for key, _label in _TOTAL_PR_COLS_WEB])

    return jsonify({
        "headers": [label for _key, label in _TOTAL_PR_COLS_WEB],
        "linhas": linhas,
    })

@app.route('/painel/dados/medias_geral')
def painel_dados_medias_geral():
    safra = request.args.get('safra')
    query = FormularioSoja.query.order_by(FormularioSoja.id)
    if safra:
        query = query.filter_by(safra=safra)
    ...
    col_map, linhas_calc = _calcular_medias_geral(registros)

    colunas = []
    for regiao, bt in col_map:
        nome_regiao = regiao if regiao else "PARANÁ"
        sub = "Total" if bt is None else ("Cultivares Bt" if bt == "SIM" else "Cultivares Não Bt")
        colunas.append(f"{nome_regiao} - {sub}")

    linhas = []
    for label, valores in linhas_calc:
        if label is None:
            linhas.append(None)  # linha em branco / separador
        else:
            linhas.append({"label": label, "valores": valores})

    return jsonify({"colunas": colunas, "linhas": linhas})

@app.route('/painel/dados/contagem_pragas')
def painel_dados_contagem_pragas():
    safra = request.args.get('safra')
    query = FormularioSoja.query.order_by(FormularioSoja.id)
    if safra:
        query = query.filter_by(safra=safra)
    ...
    headers, linhas = _calcular_contagem_pragas(registros)
    return jsonify({"headers": headers, "linhas": linhas})

@app.route('/painel/dados/contagem_doencas')
def painel_dados_contagem_doencas():
    headers, linhas = _calcular_contagem_doencas(_registros_da_safra())
    return jsonify({"headers": headers, "linhas": linhas})

@app.route('/painel/dados/tabelas_graficos_mip')
def painel_dados_tabelas_graficos_mip():
    resultado = _calcular_grafico_mip(_registros_da_safra())
    return jsonify(resultado)

@app.route('/painel/dados/tabelas_graficos_mid')
def painel_dados_tabelas_graficos_mid():
    resultado = _calcular_grafico_mid(_registros_da_safra())
    return jsonify(resultado)

def _registros_da_safra():
    """Registros (como dict) filtrados pela safra na querystring, se houver."""
    safra = request.args.get('safra')
    query = FormularioSoja.query.order_by(FormularioSoja.id)
    if safra:
        query = query.filter_by(safra=safra)
    return [orm_para_dict(r) for r in query.all()]

def _resposta_distribuicao(lista_alvo):
    resultado = _calcular_distribuicao_alvo(_registros_da_safra(), lista_alvo)
    return jsonify({
        "colunas": [f"{(r or 'PARANÁ')} - {('Total' if bt is None else ('Bt' if bt=='SIM' else 'Não Bt'))}"
                    for r, bt in resultado["col_map"]],
        "aplicacoes": resultado["aplicacoes"],
        "dae": resultado["dae"],
    })

@app.route('/painel/dados/lagartas')
def painel_dados_lagartas():
    return _resposta_distribuicao(_LAGARTAS)

@app.route('/painel/dados/percevejos')
def painel_dados_percevejos():
    return _resposta_distribuicao(_PERCEVEJOS)

@app.route('/painel/dados/op_acaros')
def painel_dados_op_acaros():
    return _resposta_distribuicao(_OUTRAS_PRAGAS + ACAROS)

def _resposta_blocos(fn_calc):
    resultado = fn_calc(_registros_da_safra())
    colunas = [f"{(r or 'PARANÁ')} - {('Total' if bt is None else ('Bt' if bt=='SIM' else 'Não Bt'))}"
               for r, bt in resultado["col_map"]]
    return jsonify({"colunas": colunas, "blocos": resultado["blocos"]})

@app.route('/painel/dados/fbn')
def painel_dados_fbn():
    return _resposta_blocos(_calcular_fbn)

@app.route('/painel/dados/tto_sal_cb')
def painel_dados_tto_sal_cb():
    return _resposta_blocos(_calcular_tto_sal_cb)

@app.route('/painel/dados/ins_dess')
def painel_dados_ins_dess():
    return _resposta_blocos(_calcular_ins_dess)

@app.route('/painel/dados/herbicidas')
def painel_dados_herbicidas():
    return _resposta_blocos(_calcular_herbicidas)

@app.route('/painel/dados/doencas')
def painel_dados_doencas():
    return _resposta_distribuicao(DOENCAS)
